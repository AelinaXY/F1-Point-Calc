...
public class RawDataCalculation extends AbstractCalculation {
    ...
    private void teamLoop(
        ...
        if (previousLevelTeamSet.size() == 2) {
            ScoreCard scoreCard = new ScoreCard(new ArrayList<>(driverSet), new ArrayList<>(previousLevelTeamSet), getRaceName(), getCostCap(), isSprint(), null, 0,0);
    ...
    public ScoreCard createPreviousScoreCard(List<String> driverNames, List<String> teamNames) {
        ...
        scoreCard.intialize(getRaceName(), getCostCap(), isSprint(), null, 0, 0);
        return scoreCard;
    }
}
